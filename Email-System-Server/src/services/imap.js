...
let naiveBayes = null;
try {
  naiveBayes = require('../services/naiveBayes');
} catch (err) {
  console.warn('⚠ Failed to load Naive Bayes service:', err.message);
}
...
class IMAPService {
  ...
  async deliverEmailToUser(user, emailData) {
    try {
      ...
      // Encrypt sensitive content (EXCLUDING subject as requested)
      ...
      // Try to use Naive Bayes if available, otherwise fallback to local logic
      ...
      if (naiveBayes) {
        try {
          const classification = naiveBayes.classifyEmail(subject, bodyText);
          aiCategory = classification.category;
          isSpam = classification.isSpam;
          aiSpamScore = classification.spamScore;
          aiSentiment = classification.sentiment;
          usedAI = true;
          console.log('✓ AI Classified:', { aiCategory, isSpam, confidence: classification.confidence });
        } catch (error) {
      ...
      if (!usedAI) {
        ...
        const result = this.classifyEmail(senderEmail, subject, bodyText);
        aiCategory = result.aiCategory;
        isSpam = result.isSpam;
        aiSpamScore = result.aiSpamScore;
      }
      ...
      await this.supabaseAdmin
        ...
        .insert({
          id: threadId,
          user_id: user.id,
          subject: finalSubject, // Storing plain text
